...
const capGroup = document.getElementById('capG');
...
function boom() {
    const o = getOrigin();
    for (let i = 0; i < 70; i++) setTimeout(() => mkDrop(o), i * 16);
    for (let i = 0; i < 40; i++) setTimeout(() => mkBub(o), i * 20 + 25);
    for (let i = 0; i < 22; i++) setTimeout(() => mkDrop(o), 300 + i * 28);
...
function loop(ts) {
    ...
    if (!pts.length) last = null;
}

function drawDrop(p, a) {
    ctx.save();
    ctx.globalAlpha = a;
    ...
    const rx = p.r,
        ry = p.r * el;
    const g = ctx.createRadialGradient(-rx * 0.28, -ry * 0.26, 0, 0, 0, Math.max(rx, ry) * 1.1);
    const hlR = Math.min(255, p.cr + 130),
        hlG = Math.min(255, p.cg + 55),
        hlB = Math.min(255, p.cb + 25);
...
if (form) {
    ...
    const passRepeatError = document.getElementById('password-repeat-error');
    ...
    [email, pass, passRepeat].forEach((field, index) => {
        if (field) {
            field.addEventListener('input', () => {
                let errorEl = null;
                if (field === email) errorEl = emailError;
                if (field === pass) errorEl = passError;
                if (field === passRepeat) errorEl = passRepeatError;
    ...
    form.addEventListener('submit', (e) => {
        ...
        setError(pass, passError, '');
        setError(passRepeat, passRepeatError, '');
        let hasErrors = false;
