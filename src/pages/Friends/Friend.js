...
export default class FriendsPage extends BasePage {
  ...
  async loadContext() {
    ...
    const q = String(nextContext.searchQuery || "").trim();
    if (q && nextContext.searchResults?.length) {
      nextContext.searchResults = enrichSearchResults(
        nextContext.searchResults.map((row) => {
          const { isFriend, hasPendingRequest, ...rest } = row;
          return rest;
        }),
        {
          friends: nextContext.friends,
          outgoingRequests: nextContext.outgoingRequests,
        },
      );
    }
...
function enrichSearchResults(users = [], rel = {}) {
  ...
  const pendingTo = new Set(
    (rel.outgoingRequests || []).map((r) =>
      String(r.user_id ?? r.userId ?? r.to_user_id ?? r.toUserId ?? ""),
    ),
  );
  return users.map((u) => ({
    ...u,
