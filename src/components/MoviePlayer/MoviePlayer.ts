...
import { playerService } from "@/js/PlayerService.ts";
...
function normalizeOpenOptions(options: AnyRecord = {}) {
  return {
...
function normalizeMovieData(movieData: AnyRecord = {}) {
  const episodes = Array.isArray(movieData.episodes)
...
function normalizeEpisodeData(episode: AnyRecord = {}, index = 0) {
  const seasonNumber =
...
function resolveDirectPlaybackPayload(episode: AnyRecord | null = null) {
