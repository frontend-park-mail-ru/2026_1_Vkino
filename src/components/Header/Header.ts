...
import { getDisplayNameFromEmail } from "@/utils/user.ts";
import { canManageSupportTicketStatus } from "@/utils/support.ts";
import type { AnyRecord, AuthState } from "@/types/shared.ts";
...
export default class HeaderComponent extends BaseComponent {
  ...
  _buildContext(state: AuthState, currentContext: AnyRecord = {}) {
  ...
  _applyMenuState(nextState: AnyRecord) {
    const nextContext = {
      ...this.context,
      ...nextState,
    };
  ...
  _isClickInsideMenu(target: Node | null) {
...
function scrollToMainSection(target: string) {
