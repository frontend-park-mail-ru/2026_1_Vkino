...
class AuthStore {
  store: ReturnType<typeof createStore<AuthState>>;
   ...
   */
  _setGuest(error: string | null = null) {
    this._setState({
      status: "guest",
      user: null,
      error,
   ...
   */
  _setAuthenticated(user: AuthUser | AnyRecord) {
  ...
  updateUserProfile(profile: AuthUser | UserProfileDto | AnyRecord): void {
    ...
    const normalizedProfile = normalizeAuthUser(profile, {
      roleFallback: (state.user as AuthUser).role,
    });
